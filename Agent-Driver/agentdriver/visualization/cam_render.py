...
def draw_rect(axis, selected_corners, color, linewidth=4):
...
class CameraRender(BaseRender):
    ...
    def __init__(self,
        ...
        # self.cams = CAM_NAMES
        self.cams = ['CAM_FRONT']  # only front camera for simplicity
        self.show_gt_boxes = False  # show_gt_boxes
    ...
    def render_pred_track_bbox_2D(self, predicted_agent_list, sample_token, nusc, box_color=(1, 0, 0)):
        """Render bounding box for predicted tracks."""
        sample = nusc.get('sample', sample_token)
        lidar_cs_record = nusc.get('calibrated_sensor', nusc.get(
            'sample_data', sample['data']['LIDAR_TOP'])['calibrated_sensor_token'])
        for i, cam in enumerate(self.cams):
            ...
            for j, box in enumerate(box_list):
                ...
                selected_corners = np.array([[x_min, y_min],
                                             [x_max, y_min],
                                             [x_max, y_max],
                                             [x_min, y_max]])
                draw_rect(self.axes[i//3, i % 3], selected_corners, box_color, linewidth=14)
                ...
                if box.is_sdc:
                    if render_sdc:
                        self.axes[i//3, i % 3].scatter(
                            traj_points[0], traj_points[1], color=sdc_color, s=900)
                    else:
                        continue
                else:
                    # tr_id = tr_id_list[j]
                    ...
                    # c = color_mapping[tr_id % len(color_mapping)]
                    self.axes[i//3, i %
                              3].scatter(traj_points[0], traj_points[1], color=box_color, s=30)
            self.axes[i//3, i % 3].set_xlim(0, imsize[0])
            self.axes[i//3, i % 3].set_ylim(imsize[1], 0)
